<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="名称" prop="mingcheng">
          <el-select  @change="mingchengChange" v-model="ruleForm.mingcheng" placeholder="请选择名称">
            <el-option
                v-for="(item,index) in mingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.mingcheng" label="名称" prop="mingcheng">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.jiezhongbianhao!=''&&json.jiezhongbianhao){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/yiliaojigou/mingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.mingchengOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    mingchengChange () {
      this.$http({
        url: `follow/yiliaojigou/mingcheng?columnValue=`+ this.ruleForm.mingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.dizhi){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `ertongjiezhong/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.ertongjiezhongCrossAddOrUpdateFlag = false;
                  this.parent.search();
